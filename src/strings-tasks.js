...
function isString(value) {
  return (
    typeof value === 'string' ||
    (value instanceof String && typeof value.valueOf() === 'string')
  );
}
...
function removeFirstOccurrences(str, value) {
  ...
  // If the substring is found, remove it; otherwise, return the original string
  if (index !== -1) {
...
function reverseWords(str) {
  ...
  wordsArr.forEach((value) => {
    reversedArr.push(value.split('').reverse().join(''));
  });
...
function encodeToRot13(str) {
  ...
  arr.forEach((element, index) => {
    const changedCharCode = str.charCodeAt(index);
    if (
      (changedCharCode >= 97 && changedCharCode <= 109) ||
      (changedCharCode >= 65 && changedCharCode <= 77)
    ) {
      changedArr.push(String.fromCharCode(changedCharCode + 13));
    } else if (
      changedCharCode === 32 ||
      changedCharCode === 33 ||
      changedCharCode === 63
    ) {
      changedArr.push(String.fromCharCode(changedCharCode));
    } else {
      changedArr.push(String.fromCharCode(changedCharCode - 13));
    }
  });
